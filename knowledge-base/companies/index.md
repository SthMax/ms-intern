# Onshore Fund Manager AI/LLM Landscape

## Inclusion rule

Include an entity in the verified set only if it is an onshore mutual fund
management company and a public primary source identifies a concrete AI/LLM
initiative, capability, partnership, procurement, product feature, or deployment.
General statements that “AI is important” remain candidates, not verified
initiatives.

## Company database

| Company | Chinese legal name | Asset-size value | Asset metric/scope | As-of date | Size source | Initiative/use case | Category | Stage | Technology/model/partner | Deployment evidence | Primary source ID | Evidence state | Notes |
|---|---|---:|---|---|---|---|---|---|---|---|---|---|---|

Allowed category tags: `research`, `trading`, `risk`, `marketing/service`,
`compliance`, `operations`, `technology platform`, `other`.

Allowed stage values: `announcement`, `research`, `proof of concept`, `pilot`,
`limited deployment`, `production`, `unclear`.

## Coverage tracker

| Metric | Target | Current |
|---|---:|---:|
| Verified companies | 15 | 0 |
| Reserve candidates | 5 | 0 |
| Companies with verified size data | 15 | 0 |
| Companies with a direct primary disclosure | 15 | 0 |

## Interpretation guardrails

- Vendor involvement does not prove the fund manager uses the system in
  production.
- A named model or partner should be recorded only when explicitly disclosed.
- Never guess deployment topology from a product name.
- Peer activity demonstrates market interest, not compliance approval or ROI.

