---
source_id: REG-012
evidence_state: verified-primary
issuer: "NPC Standing Committee; official text republished by CNIPA"
source_type: "PRC law"
published: "Revision 2017-11-04; CNIPA repost 2019-07-29"
effective_or_status: "2018-01-01"
accessed: 2026-09-06
official_url: https://www.cnipa.gov.cn/art/2019/7/29/art_104_67809.html
reviewed_by: Codex
human_check_status: pending
---

# 中华人民共和国标准化法（2017年修订）

[Official source](https://www.cnipa.gov.cn/art/2019/7/29/art_104_67809.html).

Relevant location: Articles 2, 18, 45.

## 原始来源存档与提取说明

[原始文件](../sources/REG-012/source.html) · [完整原语言提取文本](../sources/REG-012/original.md) · [获取元数据与校验值](../sources/REG-012/metadata.json)

2026-09-07 重新获取的文件用于原文保全。以下逐字复制存档提取文本中的完整条文，未翻译、未回译；代码块保留提取格式。首次获取的提取文本出现压缩内容误解码问题，已重新获取并核对可读中文提取文本。网页版式或动态内容可能存在提取差异，以原始文件为准。本次原文保全不替代法律适用、现行状态或人工核验；先前证据状态和核验限制保留。

## 原文摘录 / Original-language excerpts

### 第二条（完整条文）

```text
第二条　本法所称标准（含标准样品），是指农业、工业、服务业以及社会事业等领域需要统一的技术要求。

标准包括国家标准、行业标准、地方标准和团体标准、企业标准。国家标准分为强制性标准、推荐性标准，行业标准、地方标准是推荐性标准。

强制性标准必须执行。国家鼓励采用推荐性标准。
```

### 第十八条（完整条文）

```text
第十八条　国家鼓励学会、协会、商会、联合会、产业技术联盟等社会团体协调相关市场主体共同制定满足市场和创新需要的团体标准，由本团体成员约定采用或者按照本团体的规定供社会自愿采用。

制定团体标准，应当遵循开放、透明、公平的原则，保证各参与主体获取相关信息，反映各参与主体的共同需求，并应当组织对标准相关事项进行调查分析、实验、论证。

国务院标准化行政主管部门会同国务院有关行政主管部门对团体标准的制定进行规范、引导和监督。
```

### 第四十五条（完整条文）

```text
第四十五条　本法自2018年1月1日起施行。
```

## 研究者分析 / Project interpretation (not source text)

Do not label AMAC's group standard as universally mandatory legislation, or as
necessarily optional for every member in every context. Verify relevant member
agreements, association rules, regulatory incorporation, and internal adoption.
This note classifies the source; it does not determine MSIM's obligations.

## 既有核验记录 / Prior verification

CNIPA's official reproduction and relevant articles inspected.
Comprehensive status/adoption review and human check remain pending.
