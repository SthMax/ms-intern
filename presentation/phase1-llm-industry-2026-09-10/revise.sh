...
if [[ $# -ne 1 ]]; then
  echo 'Usage: ./revise.sh a-new-output-filename.pptx' >&2
  exit 1
fi
...
cp "$DECK_WORKSPACE/src/revise-from-report.mjs" "$DECK_WORKSPACE/.build/revise-from-report.mjs"
"$DECK_RUNTIME_NODE" "$DECK_WORKSPACE/.build/revise-from-report.mjs"
